...
export default function App() {
  ...
  return (
    <div className="h-screen flex flex-col font-sans overflow-hidden relative bg-[#F8FAFC]" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      
      {/* 1. MODERN HEADER - Sticky Glassmorphism */}
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-xl shadow-sm border-b border-white/20 transition-all duration-300">
          ...
          <div className="flex items-center justify-between px-5 py-3">
              ...
              >
                <div className="relative">
                   <div className="w-10 h-10 rounded-[14px] overflow-hidden shadow-lg ring-2 ring-white ring-offset-2 ring-offset-rose-50 transition-transform duration-300 group-hover:scale-105 group-active:scale-95">
                     <BrandAvatar />
                   </div>
                ...
                <div>
                   <h1 className="text-lg font-black text-gray-900 tracking-tight leading-none group-hover:text-rose-600 transition-colors">Foodi Burger</h1>
                   <div className="flex items-center gap-1.5 mt-1">
                      <span className="w-1.5 h-1.5 rounded-full bg-green-500"></span>
                      <span className="text-[11px] font-bold text-gray-400 uppercase tracking-wide opacity-80">{t.online}</span>
                   </div>
                </div>
              </div>
              ...
              <div className="relative">
                 <button 
                   ...
                   className="flex items-center gap-2 h-9 pl-2 pr-3.5 rounded-full bg-white border border-gray-100 shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:shadow-md hover:border-rose-100 transition-all active:scale-95 group"
                 >
                 ...
                 {isLangMenuOpen && (
                    <>
                        ...
                        <div className="absolute top-full right-0 mt-2 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden min-w-[140px] animate-fade-in z-50 flex flex-col py-1">
                            <button onClick={() => changeLanguage('ru')} className={`px-4 py-2.5 text-left text-xs font-bold hover:bg-gray-50 flex items-center justify-between transition-colors ${language === 'ru' ? 'text-rose-600 bg-rose-50' : 'text-gray-700'}`}>
                                <div className="flex items-center gap-2"><span>🇷🇺</span> Русский</div>
                                {language === 'ru' && <Check className="w-3 h-3" />}
                            </button>
                            <button onClick={() => changeLanguage('en')} className={`px-4 py-2.5 text-left text-xs font-bold hover:bg-gray-50 flex items-center justify-between transition-colors ${language === 'en' ? 'text-rose-600 bg-rose-50' : 'text-gray-700'}`}>
                                <div className="flex items-center gap-2"><span>🇺🇸</span> English</div>
                                {language === 'en' && <Check className="w-3 h-3" />}
                            </button>
                            <button onClick={() => changeLanguage('ar')} className={`px-4 py-2.5 text-left text-xs font-bold hover:bg-gray-50 flex items-center justify-between transition-colors ${language === 'ar' ? 'text-rose-600 bg-rose-50' : 'text-gray-700'}`}>
                 ...
                 {(['dine-in', 'takeaway', 'delivery'] as const).map((mode) => (
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-[11px] font-extrabold transition-all duration-300 relative z-10 ${
                        orderMode === mode 
                          ? 'bg-white text-rose-600 shadow-[0_2px_10px_rgba(0,0,0,0.05)] scale-[1.02] ring-1 ring-black/5' 
                          : 'text-gray-400 hover:text-gray-600 hover:bg-white/50'
                      }`}
                    >
                      {mode === 'dine-in' && <Utensils className={`w-3.5 h-3.5 ${orderMode === mode ? 'fill-rose-600/10 stroke-[2.5px]' : ''}`} />}
                      {mode === 'takeaway' && <Footprints className={`w-3.5 h-3.5 ${orderMode === mode ? 'fill-rose-600/10 stroke-[2.5px]' : ''}`} />}
                      {mode === 'delivery' && <Bike className={`w-3.5 h-3.5 ${orderMode === mode ? 'fill-rose-600/10 stroke-[2.5px]' : ''}`} />}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
        <div className="flex justify-center mb-6">
           <span className="text-[10px] font-bold text-gray-400 bg-white/60 border border-white/40 backdrop-blur-sm px-3 py-1 rounded-full uppercase tracking-wider shadow-sm">{t.today}</span>
        </div>
      ...
      <div className="glass border-t border-white/60 p-3 pb-6 z-20">
         <div className="flex items-end gap-2 max-w-3xl mx-auto">
            <button 
               onClick={() => handleUserAction('show_categories')}
               className="h-12 w-12 shrink-0 rounded-full text-gray-500 hover:bg-rose-50 hover:text-rose-500 flex items-center justify-center transition-all duration-300"
               title={t.showMenu}
            >
               <BookOpen className="w-6 h-6" />
            </button>

            <div className="flex-1 bg-gray-100/70 rounded-3xl px-5 py-3 flex items-center focus-within:bg-white focus-within:ring-2 focus-within:ring-rose-200 transition-all border border-transparent focus-within:border-rose-200 shadow-inner">
               <input 
                 type="text" 
            ...
            {inputValue.trim() ? (
               <button 
                  ...
                  className={`h-12 w-12 shrink-0 rounded-full ${THEME.gradient} text-white flex items-center justify-center shadow-lg shadow-rose-200 active:scale-95 transition-transform`}
               >
               ...
               <button 
                  ...
                  className={`h-12 w-12 shrink-0 rounded-full ${cartCount > 0 ? THEME.gradient + ' text-white shadow-lg shadow-rose-200' : 'bg-gray-100 text-gray-500'} flex items-center justify-center transition-all active:scale-95 relative`}
               >
                  <ShoppingCart className="w-6 h-6" />
                  {cartCount > 0 && (
                     <span className="absolute -top-1 -right-1 bg-white text-rose-600 text-[10px] font-extrabold h-5 w-5 rounded-full flex items-center justify-center shadow-sm border border-rose-100">
                        {cartCount}
                     </span>
                  )}
               </button>
